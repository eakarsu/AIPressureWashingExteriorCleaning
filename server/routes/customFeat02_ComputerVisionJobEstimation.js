...
// Customer uploads photos/video; AI estimates job scope (surface area, dirt level, special requirements); generates accurate quote
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPressureWashingExteriorCleaning / cf-computer-vision-job-estimation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_job_estimation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-computer-vision-job-estimation feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision job estimation'.
Context: Customer uploads photos/video; AI estimates job scope (surface area, dirt level, special requirements); generates accurate quote
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-job-estimation/run error:', err);
