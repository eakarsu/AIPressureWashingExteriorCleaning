...
// Historical booking patterns + weather forecasts; AI predicts busy periods; suggests pricing adjustments and crew scheduling
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPressureWashingExteriorCleaning / cf-seasonal-demand-forecasting',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_seasonal_demand_forecasting`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-seasonal-demand-forecasting feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Seasonal demand forecasting'.
Context: Historical booking patterns + weather forecasts; AI predicts busy periods; suggests pricing adjustments and crew scheduling
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-seasonal-demand-forecasting/run error:', err);
