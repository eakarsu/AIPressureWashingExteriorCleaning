...
// Quote → accept → crew assignment → route optimization → real-time tracking → completion → follow-up → repeat business automation; AI optimizes all steps
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPressureWashingExteriorCleaning / cf-agentic-job-orchestration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_job_orchestration`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-job-orchestration feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic job orchestration'.
Context: Quote → accept → crew assignment → route optimization → real-time tracking → completion → follow-up → repeat business automation; AI optimizes all steps
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-job-orchestration/run error:', err);
