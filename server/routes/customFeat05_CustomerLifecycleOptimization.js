...
// New customer → retention → referral generation; AI identifies at-risk customers, timing for follow-up service, optimal referral incentives
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPressureWashingExteriorCleaning / cf-customer-lifecycle-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_customer_lifecycle_optimization`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-customer-lifecycle-optimization feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Customer lifecycle optimization'.
Context: New customer → retention → referral generation; AI identifies at-risk customers, timing for follow-up service, optimal referral incentives
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-customer-lifecycle-optimization/run error:', err);
