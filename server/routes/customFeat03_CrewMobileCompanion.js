...
// Field app with job details, before/after photo capture, real-time routing, weather alerts, safety protocols; AI provides real-time guidance
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPressureWashingExteriorCleaning / cf-crew-mobile-companion',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_crew_mobile_companion`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-crew-mobile-companion feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Crew mobile companion'.
Context: Field app with job details, before/after photo capture, real-time routing, weather alerts, safety protocols; AI provides real-time guidance
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-crew-mobile-companion/run error:', err);
