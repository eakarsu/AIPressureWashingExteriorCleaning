...
// Gap Feature: Equipment without \'/equipment
// Equipment without /equipment-maintenance-predict (schedule maintenance)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPressureWashingExteriorCleaning / gap-equipment-without-equipment',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Equipment without \'/equipment'.\nProject: AIPressureWashingExteriorCleaning.\nContext: Equipment without /equipment-maintenance-predict (schedule maintenance)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['equipment-without-equipment', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'equipment-without-equipment', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
