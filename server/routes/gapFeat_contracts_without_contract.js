...
// Gap Feature: Contracts without \'/contract
// Contracts without /contract-upsell-opportunity (identify add-on services)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPressureWashingExteriorCleaning / gap-contracts-without-contract',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Contracts without \'/contract'.\nProject: AIPressureWashingExteriorCleaning.\nContext: Contracts without /contract-upsell-opportunity (identify add-on services)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['contracts-without-contract', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'contracts-without-contract', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
