...
// Gap (AI): Contracts without '/contract
// Contracts without /contract-upsell-opportunity (identify add-on services)
import React, { useState } from 'react';

export default function GapContractsWithoutContractPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-contracts-without-contract/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Contracts without '/contract</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Contracts without /contract-upsell-opportunity (identify add-on services)</p>
