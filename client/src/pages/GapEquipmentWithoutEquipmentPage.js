...
// Gap (AI): Equipment without '/equipment
// Equipment without /equipment-maintenance-predict (schedule maintenance)
import React, { useState } from 'react';

export default function GapEquipmentWithoutEquipmentPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-equipment-without-equipment/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Equipment without '/equipment</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Equipment without /equipment-maintenance-predict (schedule maintenance)</p>
