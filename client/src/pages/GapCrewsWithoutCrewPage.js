...
// Gap (AI): Crews without '/crew
// Crews without /crew-productivity-optimize (balance workload)
import React, { useState } from 'react';

export default function GapCrewsWithoutCrewPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-crews-without-crew/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Crews without '/crew</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Crews without /crew-productivity-optimize (balance workload)</p>
